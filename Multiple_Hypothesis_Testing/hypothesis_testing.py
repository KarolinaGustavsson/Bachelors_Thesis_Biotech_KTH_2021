...
print(qv) #det här tar jättelång tid, oklart om det funkar, ska ge signifikans för alla gener
...
sns.set_style("white")
sns.set_context("talk")
sns.relplot(data=pvalues,x="log_FC",y="-log_p",hue="Significant",aspect=1.5,height=6)
plt.axhline(p_treshold)
# sns.lineplot([-6,4],[p_treshold,p_treshold],ax=ax)
# ax.set(xlabel="$log_2(TN/not TN)$", ylabel="$-log_{10}(p)$");
plt.xlabel("$log_2(TN/not TN)$")
